/// Session-context detection for herdr-mcp.
///
...
mod tests {
    ...
    fn inside_herdr_true_when_set_to_1() {
        ...
        unsafe {
            std::env::set_var(HERDR_ENV_VAR, HERDR_ENV_VALUE);
        }
        assert!(inside_herdr());
        // Restore
        match saved {
            Some(v) => unsafe {
                std::env::set_var(HERDR_ENV_VAR, v);
            },
            None => unsafe {
                std::env::remove_var(HERDR_ENV_VAR);
            },
        }
    }
    ...
    fn inside_herdr_false_when_wrong_value() {
        ...
        let saved = std::env::var(HERDR_ENV_VAR).ok();
        unsafe {
            std::env::set_var(HERDR_ENV_VAR, "0");
        }
        assert!(!inside_herdr());
        unsafe {
            std::env::set_var(HERDR_ENV_VAR, "production");
        }
        assert!(!inside_herdr());
        match saved {
            Some(v) => unsafe {
    ...
    fn pane_id_returns_some_when_set() {
        ...
        unsafe {
            std::env::set_var("HERDR_PANE_ID", "p1");
        }
    ...
    fn socket_path_override_parses_path() {
        ...
        unsafe {
            std::env::set_var("HERDR_SOCKET_PATH", "/tmp/test.sock");
        }
        assert_eq!(
        ...
        match saved {
            Some(v) => unsafe {
                std::env::set_var("HERDR_SOCKET_PATH", v);
            },
            None => unsafe {
